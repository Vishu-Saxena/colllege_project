...
if(isset($_POST['name'])){
	...
	$html = '<body>';	
		...
		$html.= '<style>.center2{text-align:center;}</style>';
		$html.='<style>td{padding-top:10px; padding-bottom:10px} </style>';
		$html.='<style>.sign{padding-left:40px;} </style>';
		$html.='<style>.colllegename{font-size:20px;} </style>';
		$html.='<style>.wish{text-align:justify;font-size:14px;} </style>';
		$html.='<style>.p{font-size:"10px";} , .signimg{margin-bottom:25px; margin-right:-150px;} </style>';
		...
		$html.='<table><tr><td class="center2"><P class="colllegename"><b> BHASKARACHARYA COLLEGE OF APPLIED SCIENCES </b></P>  <P class="p">(University of Delhi)</P> <p class="p"> Sector-2, Dwarka, New Delhi-110075 </p> <P class="p"> Accredited "A" Grade by NAAC. </P><td/> <tr/></table>';
		
		$html.='<table><tr><td class="cn">Certificate Number: BCAS/CLC/2022/'.$rollno.'<td/> <td>Date : '.$date.'<td/> <tr/>';
		$html.='</table>';
		...
		$html.='<tr><td class="sign"> <img src="csign.jpg" class="signimg"/> <b> Principal (Officiating) </b><td/><tr/>';
		$html.='<hr>';
	...
	// $mail->addAddress($_POST['email']);
	// $mail->isHTML(true);
	// $mail->Subjet="Provisional certificate";
	// $mail->Body="Provisional certificate";
